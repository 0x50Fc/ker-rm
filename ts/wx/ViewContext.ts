...
export class ViewContext {
    ...
    create(name: string, id: string): string {
        ...
        var v = app.createView(name, undefined) as View;
        v.id = id;
        v.name = name;
        this._viewSet[id] = v;
        return id;
    ...
    getCanvasContext(canvasId: string): UICanvasContext {
    ...
    setContentSizee(id: string, width: number, height: number): void {
        var v = this._viewSet[id];
        if (v) {
    ...
    off(id: string, name: string, func: EventFunction): void {
        var v = this._viewSet[id];
        if (v) {
            v.off(name, func);
        }
